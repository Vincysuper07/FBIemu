...
void action_install_titledb(linked_list* items, list_item* selected, bool cia) {
    ...
    if(data->cia) {
        snprintf(urls, sizeof(urls), "https://tdb3ds.emugamesita.ml/v1/cia/%lu/download", info->cia.id);
    } else {
        ...
        urlsPos += snprintf(urls + urlsPos, sizeof(urls) - urlsPos, "https://tdb3ds.emugamesita.ml/v1/tdsx/%lu/download\n", info->tdsx.id);
        ...
        if(info->tdsx.smdh.exists) {
            ...
            snprintf(urls + urlsPos, sizeof(urls) - urlsPos, "https://tdb3ds.emugamesita.ml/v1/smdh/%lu/download\n", info->tdsx.smdh.id);
